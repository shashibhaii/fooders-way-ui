          ...
          <div id="update-item-modal" className="form-font relative rounded shadow-2xl shadow-slate-500 bg-white">
            ...
            <div className="px-4 py-5">
              <label className="relative inline-flex items-center mb-4 cursor-pointer">
                {row&&row.isEnabled === 'YES' && <input type="checkbox" onClick={() => setIsEnabled('NO')} className="sr-only peer" defaultChecked />} 
                {row&&row.isEnabled === 'NO' &&<input type="checkbox" onClick={() => setIsEnabled('YES')} className="sr-only peer" />}
